...
CREATE TABLE IF NOT EXISTS addresses (
	...
	CONSTRAINT ck_street_number_positive
		CHECK (street_number > 0),
	CONSTRAINT ck_zip_code_positive
		CHECK (zip_code > 0)
);
...
CREATE TABLE IF NOT EXISTS players_ranges (
		...
		CONSTRAINT ck_min_players_positive
			CHECK (min_players > 0),
		CONSTRAINT ck_max_players_positive
			CHECK (max_players > 0)
);
...
CREATE TABLE IF NOT EXISTS board_games (
	...
	CONSTRAINT ck_release_year_positive
...
CREATE TABLE IF NOT EXISTS creators_board_games (
	...
	CONSTRAINT fk_creator_board_games_creators
		FOREIGN KEY (creator_id)
		REFERENCES creators(id)
		ON DELETE CASCADE
		ON UPDATE CASCADE,
	CONSTRAINT fk_creator_board_games_board_games
